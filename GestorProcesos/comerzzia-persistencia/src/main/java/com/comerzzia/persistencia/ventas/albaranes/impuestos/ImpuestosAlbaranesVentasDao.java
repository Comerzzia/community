...
package com.comerzzia.persistencia.ventas.albaranes.impuestos;
...
public class ImpuestosAlbaranesVentasDao extends MantenimientoDao {
	
	private static String TABLA = "D_CLIE_ALBARANES_IMP_TBL";
	
	private static Logger log = Logger.getMLogger(ImpuestosAlbaranesVentasDao.class);
	...
	public static List<ImpuestoBean> consultar(Connection conn, ConfigEmpresaBean config, Long idAlbaran) throws SQLException {
    	...
    	sql = "SELECT ID_CLIE_ALBARAN, CODIMP, BASE, IMPUESTOS, TOTAL " +
    	...
    	try {
        	...
        	while (rs.next()){
        		...
        		impuesto.setIdDocumento(rs.getLong("ID_CLIE_ALBARAN"));
	...
	public static void insert(Connection conn, ConfigEmpresaBean config, ImpuestoBean impuesto) throws SQLException {
		...
		sql = "INSERT INTO " + getNombreElemento(config.getEsquemaEmpresa(), TABLA) + 
		      "(ID_CLIE_ALBARAN, CODIMP, BASE, IMPUESTOS, TOTAL) VALUES " +
		      "(?, ?, ?, ?, ?)";
