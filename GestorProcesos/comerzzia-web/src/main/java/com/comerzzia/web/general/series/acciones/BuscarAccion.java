...
public class BuscarAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("general/series/buscar/jsp/buscar.jsp", Vista.INTERNA);
	...
	public Vista ejecutar(HttpServletRequest request) {
		...
		try {
    		...
    		ParametrosBuscarSeriesBean param = (ParametrosBuscarSeriesBean) sesion.getAttribute(WebKeys.PARAM_BUSCAR_SERIES);
            ...
            if (param.getNumPagina() > 0) {
            	PaginaResultados paginaResultados = ServicioSeries.consultar(param, datosSesion);
    ...
    private void setParametrosBuscar(ParametrosBuscarSeriesBean param, HttpServletRequest request) {
    ...
    private void setPaginaBuscar(ParametrosBuscarSeriesBean param, HttpServletRequest request) {
        try {
    ...
    private void setOrdenBuscar(ParametrosBuscarSeriesBean param, HttpServletRequest request) {
    	try {
            ...
            switch (columna) {
                case 1:  // CODSERIE
                    ...
                    break;
                case 2:  // DESSERIE
                    if (param.getOrden().equals("DESSERIE")) {
                        param.setOrden("DESSERIE DESC");
                    }
                    else {
                        param.setOrden("DESSERIE");
                    }
                    break;
                default:
                    param.setOrden("DESSERIE");
