...
public class BuscarAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("general/bancos/buscar/jsp/buscar.jsp", Vista.INTERNA);
	...
	public Vista ejecutar(HttpServletRequest request) {
		try {
    		...
    		ParametrosBuscarBancosBean param = (ParametrosBuscarBancosBean) sesion.getAttribute(WebKeys.PARAM_BUSCAR_BANCOS);
            ...
            if (param.getNumPagina() > 0) {
            	PaginaResultados paginaResultados = ServicioBancos.consultar(param, datosSesion);
	...
	private void setParametrosBuscar(ParametrosBuscarBancosBean param, HttpServletRequest request) {
		param.setCodBan(request.getParameter("codBan"));
		param.setDesBan(request.getParameter("desBan"));
	...
	private void setPaginaBuscar(ParametrosBuscarBancosBean param, HttpServletRequest request) {
        try {
	...
	private void setOrdenBuscar(ParametrosBuscarBancosBean param, HttpServletRequest request) {
    	try {
            ...
            switch (columna) {
                case 1:  // CODBAN
                    ...
                    break;
                case 2:  // DESBAN
                    if (param.getOrden().equals("DESBAN")) {
                        param.setOrden("DESBAN DESC");
                    }
                    else {
                        param.setOrden("DESBAN");
                    }
                    break;   
                default:
                    param.setOrden("DESBAN");
        	...
        	// Si no tenemos orden, se deja el que se tenía
        }
    }
}
