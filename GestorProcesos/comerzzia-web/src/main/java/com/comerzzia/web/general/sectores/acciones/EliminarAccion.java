...
public class EliminarAccion extends Accion {
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	...
    	try {
    		...
    		ServicioSectores.eliminar(codSec, datosSesion);
    		
    		setMensaje(request, "El sector se ha borrado correctamente");
    		
    		return getControlador().getAccion("buscar").ejecutar(request);
    	}
    	catch (SectorConstraintViolationException e) {
    		setMensajeError(request, "No se ha podido borrar el sector porque existen registros asociados");
    		
    		// Si se estaba consultando volvemos a mostrar el registro
    		if (sesion.getAttribute(WebKeys.SECTOR_BEAN) != null) {
    			return NEXT_PAGE;
    		}
