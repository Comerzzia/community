...
public class BuscarAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("core/perfiles/buscar/jsp/buscar.jsp", Vista.INTERNA);
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	try {
    		...
    		ParametrosBuscarPerfilesBean param = (ParametrosBuscarPerfilesBean) sesion.getAttribute(WebKeys.PARAM_BUSCAR_PERFILES);
            ...
            if (param.getNumPagina() > 0) {
            	PaginaResultados paginaResultados = ServicioPerfiles.consultar(param);
    ...
    private void setParametrosBuscar(ParametrosBuscarPerfilesBean param, HttpServletRequest request) {
    	param.setDesPerfil(request.getParameter("desPerfil"));
    ...
    private void setPaginaBuscar(ParametrosBuscarPerfilesBean param, HttpServletRequest request) {
        try {
    ...
    private void setOrdenBuscar(ParametrosBuscarPerfilesBean param, HttpServletRequest request) {
    	try {
            ...
            switch (columna) {
                case 1:  // DESPERFIL
                    if (param.getOrden().equals("DESPERFIL")) {
                        param.setOrden("DESPERFIL DESC");
                    }
                    else {
                        param.setOrden("DESPERFIL");
                    }
                    break;
                default:
                    param.setOrden("DESPERFIL");
