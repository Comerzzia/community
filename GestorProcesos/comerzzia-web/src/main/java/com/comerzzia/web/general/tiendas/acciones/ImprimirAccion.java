...
public class ImprimirAccion extends InformeAccion {
	...
	public String getNombreInforme() {
		return "general.tiendas.lstTiendas";
	}
	
	public Vista nuevoInforme(HttpServletRequest request, TrabajoInformeBean trabajoInforme) {
		return new Vista("general/tiendas/buscar/jsp/imprimir.jsp", Vista.INTERNA);
	}
	...
	public void imprimirInforme(HttpServletRequest request, TrabajoInformeBean trabajoInforme) 
		...
		if (request.getParameter("codAlmDesde").isEmpty()){
			trabajoInforme.addParametro("CODALM_DESDE", "0");
		}
		else{
			trabajoInforme.addParametro("CODALM_DESDE", request.getParameter("codAlmDesde"));
		}
		if (request.getParameter("codAlmHasta").isEmpty()){
			trabajoInforme.addParametro("CODALM_HASTA", "ZZZZ");
		}
		else{
			trabajoInforme.addParametro("CODALM_HASTA", request.getParameter("codAlmHasta"));
		}
	}
