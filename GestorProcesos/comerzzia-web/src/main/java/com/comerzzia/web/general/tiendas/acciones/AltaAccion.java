...
public class AltaAccion extends Accion {
	...
	public Vista ejecutar(HttpServletRequest request) {
		try {
			...
			AsistenteTienda asistente = new AsistenteTienda();
			formulario.setModoInsercion(asistente.getTienda());
			
			sesion.setAttribute(WebKeys.ASISTENTE_TIENDA, asistente);

			// Mostramos el formulario
			return getControlador().getAccion("asistenteAlta").ejecutar(request);
		} catch (Exception e) {
			e.printStackTrace();
			setError(request, e);
