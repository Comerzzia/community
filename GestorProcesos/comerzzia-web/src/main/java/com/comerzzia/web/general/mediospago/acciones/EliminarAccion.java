...
public class EliminarAccion extends Accion {
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	...
    	try {
    		...
    		String codMedioPago = request.getParameter(WebKeys.ID_OBJETO);
    		ServicioMediosPago.eliminar(codMedioPago, datosSesion);
    		
    		setMensaje(request, "El medio de pago se ha borrado correctamente");
    		
    		return getControlador().getAccion("buscar").ejecutar(request);
    	}
    	catch (MedioPagoConstraintViolationException e) {
    		setMensajeError(request, "No se ha podido borrar el medio de pago porque existen registros asociados");
