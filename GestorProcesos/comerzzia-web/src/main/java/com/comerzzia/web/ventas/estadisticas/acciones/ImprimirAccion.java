...
package com.comerzzia.web.ventas.estadisticas.acciones;
...
public class ImprimirAccion extends InformeAccion {
	...
	public String getNombreInforme() {
		return "ventas.estadisticas.lstSituacionVentas";
	}
	
	@Override
	public Vista nuevoInforme(HttpServletRequest request,
			TrabajoInformeBean trabajoInforme) {
		return new Vista("ventas/estadisticas/buscar/jsp/imprimir.jsp", Vista.INTERNA);
	}
	...
	public void imprimirInforme(HttpServletRequest request,
		...
		//Clientes
		if (request.getParameter("codCliDesde").isEmpty()){
			trabajoInforme.addParametro("CODCLI_DESDE", "0");
		}
		else{
			trabajoInforme.addParametro("CODCLI_DESDE", request.getParameter("codCliDesde"));
		}
		if (request.getParameter("codCliHasta").isEmpty()){
			trabajoInforme.addParametro("CODCLI_HASTA", "ZZZZ");
		}
		else{
			trabajoInforme.addParametro("CODCLI_HASTA", request.getParameter("codCliHasta"));
		}
