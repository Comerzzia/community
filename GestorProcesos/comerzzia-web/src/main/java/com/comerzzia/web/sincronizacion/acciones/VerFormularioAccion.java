...
public class VerFormularioAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("sincronizacion/jsp/sincronizador.jsp", Vista.INTERNA);
	...
	private void inicializaPestañaActiva(FormularioSincronizacionBean formulario, DatosSesionBean datosSesion, HttpServletRequest request, HttpSession sesion) throws Exception {
		if(!formulario.getFormularioPestañaActiva().isModoVisualizacionFicha()){
			...
			switch (formulario.getPestañaActiva()) {
				case 0:
					formulario.getFormularioPestañaActiva().setRegistros(ServicioSincronizador.consultarTrabajos(datosSesion));
	
					break;
				case 1:
					formulario.getFormularioPestañaActiva().setRegistros(ServicioSincronizador.consultarTrabajosTiendas(datosSesion));
