...
public class VerFormularioAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("general/medios_pago/mantenimiento/jsp/medioPago.jsp", Vista.INTERNA);
	...
	private void inicializaPestañaActiva(FormularioMedioPagoBean formulario, DatosSesionBean datosSesion) throws Exception {
		...
		switch (formulario.getPestañaActiva()) {
			case 1:
				if (!medioPago.isVencimientosCargados()){
					medioPago.setVencimientos(ServicioVencimientos.consultarVencimientos(medioPago.getCodMedioPago(), datosSesion));
					medioPago.setVencimientosCargados(true);
					formulario.getFormularioPestañaActiva().setRegistros(medioPago.getVencimientos());
				}
				break;
		}
	}
