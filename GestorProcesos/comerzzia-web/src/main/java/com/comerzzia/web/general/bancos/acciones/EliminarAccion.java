...
public class EliminarAccion extends Accion {
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	...
    	try {
    		...
    		ServicioBancos.eliminar(codBan, datosSesion);
    		
    		setMensaje(request, "El banco se ha borrado correctamente");
    		
    		return getControlador().getAccion("buscar").ejecutar(request);
    	}
    	catch (BancoConstraintViolationException e) {
    		setMensajeError(request, "No se ha podido borrar el banco porque existen registros asociados");
    		
    		// Si se estaba consultando volvemos a mostrar el registro
    		if (sesion.getAttribute(WebKeys.BANCO_BEAN) != null) {
    			return NEXT_PAGE;
    		}
