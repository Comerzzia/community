...
public class EliminarAccion extends Accion {
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	...
    	try {
    		...
    		Long idTipoPorte = Long.parseLong(request.getParameter(WebKeys.ID_OBJETO));
    		ServicioTiposPortes.eliminar(idTipoPorte, datosSesion);
    		
    		setMensaje(request, "El tipo de porte se ha borrado correctamente");
    		
    		return getControlador().getAccion("buscar").ejecutar(request);
    	}
    	catch (TipoPorteConstraintViolationException e) {
    		setMensajeError(request, "No se ha podido borrar el tipo Porte porque existen registros asociados");
    		
    		// Si se estaba consultando volvemos a mostrar el registro
    		if (sesion.getAttribute(WebKeys.TIPO_PORTE_BEAN) != null) {
    			return NEXT_PAGE;
    		}
