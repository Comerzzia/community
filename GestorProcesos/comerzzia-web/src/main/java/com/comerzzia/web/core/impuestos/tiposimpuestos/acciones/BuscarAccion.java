...
public class BuscarAccion extends Accion {
	...
	private static final Vista NEXT_PAGE = new Vista("core/impuestos/tipos_impuestos/buscar/jsp/buscar.jsp", Vista.INTERNA);
	...
	public Vista ejecutar(HttpServletRequest request) {
		...
		try {
    		...
    		ParametrosBuscarTiposImpuestosBean param = (ParametrosBuscarTiposImpuestosBean) sesion.getAttribute(WebKeys.PARAM_BUSCAR_TIPOS_IMPUESTOS);
            ...
            if (param.getNumPagina() > 0) {
            	PaginaResultados paginaResultados = ServicioTiposImpuestos.consultar(param);
    ...
    private void setParametrosBuscar(ParametrosBuscarTiposImpuestosBean param, HttpServletRequest request) {
    	param.setCodTipoImpuesto(request.getParameter("codTipoImpuesto"));
        param.setDesTipoImpuesto(request.getParameter("desTipoImpuesto"));
    ...
    private void setPaginaBuscar(ParametrosBuscarTiposImpuestosBean param, HttpServletRequest request) {
        try {
    ...
    private void setOrdenBuscar(ParametrosBuscarTiposImpuestosBean param, HttpServletRequest request) {
    	try {
            ...
            switch (columna) {
                case 1:  // CODIMP
                    if (param.getOrden().equals("CODIMP")) {
                        param.setOrden("CODIMP DESC");
                    }
                    else {
                        param.setOrden("CODIMP");
                    }
                    break;
                case 2:  // DESIMP
