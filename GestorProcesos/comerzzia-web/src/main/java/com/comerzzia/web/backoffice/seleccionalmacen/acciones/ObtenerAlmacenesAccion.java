...
public class ObtenerAlmacenesAccion extends Accion {

	private static final Vista NEXT_PAGE = new Vista(
			"backoffice/seleccionAlmacen/jsp/seleccionAlmacen.jsp",
			Vista.INTERNA);
	...
	public Vista ejecutar(HttpServletRequest request) {
		try {
			...
			if (numeroTiendas > 1) {
				...
				request.setAttribute("listaAlmacenesUsuario", almacenes);

				return NEXT_PAGE;
			} else if (numeroTiendas == 1) {
