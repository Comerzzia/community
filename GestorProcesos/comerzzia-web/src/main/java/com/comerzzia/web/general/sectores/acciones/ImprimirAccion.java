...
public class ImprimirAccion extends InformeAccion {
	...
	public String getNombreInforme() {
		return "general.sectores.lstSectores";
	}
	
	public Vista nuevoInforme(HttpServletRequest request, TrabajoInformeBean trabajoInforme) {
		return new Vista("general/sectores/buscar/jsp/imprimir.jsp", Vista.INTERNA);
	}
	...
	public void imprimirInforme(HttpServletRequest request, TrabajoInformeBean trabajoInforme) 
		...
		if (request.getParameter("codSecDesde").isEmpty()){
			trabajoInforme.addParametro("CODSEC_DESDE", "0");
		}
		else{
			trabajoInforme.addParametro("CODSEC_DESDE", request.getParameter("codSecDesde"));
		}
		
		if (request.getParameter("codSecHasta").isEmpty()){
			trabajoInforme.addParametro("CODSEC_HASTA", "ZZZZ");
		}
		else{
			trabajoInforme.addParametro("CODSEC_HASTA", request.getParameter("codSecHasta"));
		}
	}
}
