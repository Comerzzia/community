...
public class EjecutarAccion extends Accion {
    ...
    public Vista ejecutar(HttpServletRequest request) {
    	try {
    		...
    		if (param == null) {
    			param = new ParametrosBuscarFacturasVentasBean();
    			Date hoy = new Date();
				param.setFechaDesde(Fechas.sumaDias(hoy, -30));
				param.setFechaHasta(hoy);
    			
    			sesion.setAttribute(WebKeys.PARAM_BUSCAR_FACTURAS_VENTAS, param);
    		}
    		else {
    		...
    		if (formulario == null) {
    			formulario = new FormularioFacturaVentaBean();
    			...
    			sesion.setAttribute(WebKeys.FORMULARIO_FACTURA_VENTA, formulario);
    		}
