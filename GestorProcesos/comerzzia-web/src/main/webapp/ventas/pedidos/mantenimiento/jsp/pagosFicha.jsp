...
<c:set var="pago" value="${formularioPedidoVenta.formularioPestañaActiva.registroActivo}" />
  ...
  <cmz:cuerpoPanel>
    <table cellpadding="2px" cellspacing="2px" border="0px">
      <tr>
        <td>Medio de Pago:</td>
        <td>
          <cmz:ayuda nombreAyuda="MEDIOS_PAGO_VEN_VENTAS" requerido="true" soloLectura="${!formulario.editable}">
          ...
          <cmz:campoTexto id="importe" requerido="true" editable="${formulario.editable}" soloLectura="${!formulario.editable}" anchura="90px">
            <c:choose>
              <c:when test="${formulario.enInsercion}">
                <cmz:formateaNumero numDecimales="2" valor="${pedido.restantePagos}"/>
              </c:when>
              <c:otherwise>
