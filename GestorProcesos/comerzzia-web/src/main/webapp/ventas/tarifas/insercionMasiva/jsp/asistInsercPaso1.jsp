...
<script type="text/javascript">
	...
	function checkForm() {
		...
		if (!isCheckTarifaGeneral()) {
			...
			var origen = document.getElementById("codTarOrigen").value;
			if (origen == actual) {
				alert("Debe seleccionar una tarifa origen diferente de la actual.");
				return false;
			} 
		} 
		return true;
...
<cmz:main>
	<cmz:panelCMZ>
		...
		<cmz:cuerpoPanelCMZ>
			...
			<input id="codTarActual" name="codTarActual" type="hidden" value="${tarifaAsistente.codTarActual}" />
			<cmz:panel>
				...
				<cmz:cuerpoPanel>
					<table cellpadding="2px" cellspacing="2px" border="0px" class="insercionMasiva">
						<c:set var="selecTarifaGeneral" value="${(tarifaAsistente.tarifaOrigenGeneral || tarifaAsistente.codTarOrigen==null) && !tarifaAsistente.tarifaActualGeneral}" />
						<c:if test="${!tarifaAsistente.tarifaActualGeneral}">
							<tr>
								<td>
								...
								<cmz:ayuda requerido="true" soloLectura="false" nombreAyuda="TARIFAS">
									<c:choose>
									  <c:when test="${!selecTarifaGeneral}">
										<cmz:codigoAyuda idCodigo="codTarOrigen" valorCodigo="${tarifaAsistente.codTarOrigen}" 
									...
									<cmz:descripcionAyuda idDescripcion="desTar" valorDescripcion="${tarifaAsistente.desTar}" 
