...
<script type="text/javascript">
    ...
    function recepcionarLineasCompleto() {
    	if(confirm("¿Desea recepcionar por completo todas las líneas no rechazadas?")) {
        	document.getElementById("accion").value = "leerFormulario";
        	document.getElementById("operacion").value = "recepcionarLineasCompleto";
        	document.getElementById("frmDatos").submit();
        	_esperando();
    	}
    }
    ...
    <cmz:acciones>
      <c:if test="${formularioPedidoCompra.editable}">
        ...
        <c:if test="${pedido.estaPendienteAceptacion}">
          <cmz:accion onclick="aceptarLineasCompleto();" descripcion="Aceptar todas las líneas" icono="compras/pedidos/buscar/images/aceptarLineas.gif"></cmz:accion>
        </c:if>
        <c:if test="${pedido.estaPendienteRecepcion}">
          <cmz:accion onclick="recepcionarLineasCompleto();" descripcion="Recepcionar todas las líneas" icono="compras/pedidos/buscar/images/recepcionarLineas.gif"></cmz:accion>
        </c:if>
        <c:if test="${!pedido.estadoNuevo && !pedido.estaSolicitud}">
          <cmz:accion onclick="rechazarLineasCompleto();" descripcion="Rechazar todas las líneas" icono="compras/pedidos/buscar/images/rechazarLineas.gif"></cmz:accion>
        </c:if>
      </c:if>
    </cmz:acciones>
  </cmz:cabeceraPanel>
  ...
  <cmz:cuerpoPanel>
    <cmz:lista>
      <cmz:cabeceraLista>
        ...
        <c:if test="${!empty formularioPedidoCompra.tituloDesglose1}">
          <cmz:itemCabeceraLista nombre="${formularioPedidoCompra.tituloDesglose1}"></cmz:itemCabeceraLista>
        </c:if>
        <c:if test="${!empty formularioPedidoCompra.tituloDesglose2}">
          <cmz:itemCabeceraLista nombre="${formularioPedidoCompra.tituloDesglose2}"></cmz:itemCabeceraLista>
        </c:if>
        ...
        <cmz:itemCabeceraLista nombre="U. Recepcionadas" estilo="text-align: right;"></cmz:itemCabeceraLista>
      ...
      <cmz:contenidoListaIndexada variable="articulo" lista="${formularioPedidoCompra.formularioPestañaActiva.registros}">
        ...
        <cmz:itemContenidoLista alineacion="right"><cmz:formateaNumero valor="${articulo.cantidadRecepcionada}"/></cmz:itemContenidoLista>
