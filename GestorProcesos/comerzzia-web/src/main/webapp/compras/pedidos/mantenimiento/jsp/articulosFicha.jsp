...
<c:set var="formulario" value="${formularioPedidoCompra.formularioPestañaActiva}" />
<c:set var="articulo" value="${formularioPedidoCompra.formularioPestañaActiva.registroActivo}" />
...
<c:choose>
  <c:when test="${pedido.estaSolicitud}">
    ...
    <c:set var="idCantidadMedidaRecepcionada" value="cantidadMedidaRecepcionada" />
    <c:set var="idUnidadesRecepcionadas" value="cantidadRecepcionada" />
    <c:set var="soloLecturaRecepcionadas" value="true" />
  ...
  <c:when test="${pedido.estaPendienteRecepcion}">
    <c:set var="idCantidadMedidaSolicitada" value="cantidadMedidaSolicitada" />
    <c:set var="idUnidadesSolicitadas" value="cantidadSolicitada" />
    <c:set var="soloLecturaSolicitadas" value="true" />
    <c:set var="idCantidadMedidaAceptada" value="cantidadMedidaAceptada" />
    <c:set var="idUnidadesAceptadas" value="cantidadAceptada" />
    <c:set var="soloLecturaAceptadas" value="true" />
    <c:set var="idCantidadMedidaRecepcionada" value="cantidadMedida" />
    <c:set var="idUnidadesRecepcionadas" value="cantidad" />
    <c:set var="soloLecturaRecepcionadas" value="${!formulario.editable}" />
    ...
    function validarArticulo() {
        ...
        if (response != null) {
            if (response.getElementsByTagName("ERROR").item(0)) {
                ...
                // Establecemos el descuento que ofrece el proveedor
                document.getElementById("descuento").value = formateaNumero(response.getElementsByTagName("dtoProveedor").item(0).firstChild.data, 2);
                
                // Establecemos el precio de costo
...
<cmz:panel>
  ...
  <cmz:cuerpoPanel>
    <table cellpadding="2px" cellspacing="2px" border="0px">
      ...
      <c:choose>
        <c:when test="${empty formularioPedidoCompra.tituloDesglose1}">
          <input name="desglose1" id="desglose1" type="hidden" value="*" />
        </c:when>
        <c:otherwise>
          <tr>
            <td><c:out value="${formularioPedidoCompra.tituloDesglose1}" />:</td>
            <td colspan="3">
              <c:choose>
      ...
      <c:choose>
        <c:when test="${empty formularioPedidoCompra.tituloDesglose2}">
          <input name="desglose2" id="desglose2" type="hidden" value="*" />
        </c:when>
        <c:otherwise>
          <tr>
            <td><c:out value="${formularioPedidoCompra.tituloDesglose2}" />:</td>
            <td colspan="3">
              <c:choose>
              	<c:when test="${campoEditable}">
      ...
      <tr>
        ...
        <td>
          <cmz:campoTexto id="${idCantidadMedidaRecepcionada}" requerido="false" soloLectura="${soloLecturaRecepcionadas}" onChange="calcularUnidades();" anchura="70px">
          	<cmz:formateaNumero valor="${articulo.cantidadMedidaRecepcionada}"/>	
          </cmz:campoTexto>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <cmz:campoTexto id="${idUnidadesRecepcionadas}" requerido="true" editable="${!soloLecturaRecepcionadas}" soloLectura="${soloLecturaRecepcionadas}" onChange="calcularImporte();" anchura="70px">
          	<cmz:formateaNumero valor="${articulo.cantidadRecepcionada}"/>	
          </cmz:campoTexto>
        </td>
      </tr>
