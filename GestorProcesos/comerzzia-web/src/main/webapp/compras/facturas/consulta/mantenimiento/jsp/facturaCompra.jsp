...
<jsp:useBean id="formularioFacturaCompra" type="com.comerzzia.web.compras.facturas.consulta.ui.FormularioFacturaCompraBean" scope="session" />
<jsp:useBean id="permisos" class="com.comerzzia.servicios.core.permisos.PermisosEfectivosAccionBean" scope="request" />

<c:set var="factura" value="${formularioFacturaCompra.registroActivo}" />
      ...
      <cmz:panelCMZ>
        ...
        <cmz:cuerpoPanelCMZ>
          ...
          <form id="frmDatos" name="frmDatos" action="facturasCompras" method="post">
                  ...
                  <tr>
                    ...
                    <td>
                      <cmz:campoTexto id="periodo" valor="${factura.periodo}" requerido="false" anchura="40px" longitudMaxima="4"
                                        editable="${formularioFacturaCompra.enInsercion}" soloLectura="${!formularioFacturaCompra.enInsercion}"/>              
					  <cmz:campoTexto id="codSerie" valor="${factura.codSerie}" requerido="false" anchura="40px" longitudMaxima="5"
                                        editable="${formularioFacturaCompra.enInsercion}" soloLectura="${!formularioFacturaCompra.enInsercion}"/>
					  <cmz:campoTexto id="numFactura" valor="${factura.numFactura}" requerido="false" anchura="90px" longitudMaxima="10" 
                                        editable="${formularioFacturaCompra.enInsercion}" soloLectura="${!formularioFacturaCompra.enInsercion}"/>
                    </td>                
                    <td align="right">Fecha:</td>
                    <td><c:set var="fecha"><fmt:formatDate pattern="dd/MM/yyyy" value="${factura.fechaRecFactura}"/></c:set>
                    <cmz:campoTexto id="fecha" valor="${fecha}" requerido="false" editable="${formularioFacturaCompra.editable}" 
                    					soloLectura="${!formularioFacturaCompra.editable}" anchura="65"/></td>
                  </tr>
                  <tr>   					
                    <td>Proveedor:</td>
                    <td colspan="3"><cmz:campoTexto id="codPro" valor="${factura.codProveedor}" requerido="false" anchura="100px" longitudMaxima="11" 
                                        editable="${formularioFacturaCompra.editable}" soloLectura="${!formularioFacturaCompra.editable}"/>
                    	<cmz:campoTexto id="desPro" valor="${factura.desProveedor}" requerido="false" anchura="300px" longitudMaxima="45" 
                                        editable="${formularioFacturaCompra.editable}" soloLectura="${!formularioFacturaCompra.editable}"/>
                    </td>
                  </tr>
                </table>
            ...
            <cmz:pestañas>
            	<cmz:cabeceraPestañas pestañaActiva="${formularioFacturaCompra.pestañaActiva}">
            	...
            	<cmz:cuerpoPestaña>
            		<c:choose>
                        <c:when test="${formularioFacturaCompra.pestañaActiva == 1}">
                        	<c:choose>
		                      <c:when test="${formularioFacturaCompra.formularioPestañaActiva.modoVisualizacionFicha}">
                        ...
                        <c:when test="${formularioFacturaCompra.pestañaActiva == 2}">
                        	<c:choose>
		                      <c:when test="${formularioFacturaCompra.formularioPestañaActiva.modoVisualizacionFicha}">
