...
public class ConfigSincronizacionMySQL extends ConfigSincronizacionBBDD implements IConfigSincronizacion {

	/**
	 * Constructor
	 */
	public ConfigSincronizacionMySQL() {
	...
	public String getUrlConexion() {
		String url = this.url + parametros.get(PARAM_SERVIDOR) + ":" 
				+ parametros.get(PARAM_PUERTO) + "/" + parametros.get(PARAM_ESQUEMA);
	...
	public void parseUrlConexion(String url) throws XMLDocumentException {
		...
		if (p != -1) {
			try {
				this.url = url.substring(0, p+2);
				String[] param = url.substring(p+2, url.length()).split("[:/]");
				parametros.put(PARAM_SERVIDOR, param[0]);
				parametros.put(PARAM_PUERTO, param[1]);
				parametros.put(PARAM_ESQUEMA, param[2]);
			} 
			catch (Exception e) {
